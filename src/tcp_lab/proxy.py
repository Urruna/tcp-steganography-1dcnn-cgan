...
def write_log(session_id: str, direction: str,
              event_index: int, byte_length: int,
              inter_event_gap_ms: float) -> None:
    with LOG_FILE.open("a", newline="", encoding="utf-8") as file:
        writer = csv.writer(file)
        writer.writerow([
            f"{time.time():.6f}",
            PROXY_NAME,EXPERIMENT_ID,
            session_id,
            direction,
            event_index,
            byte_length,
...
async def forward(source_reader: asyncio.StreamReader,
    ...
    try:
        ...
        print(f"[{PROXY_NAME}] forward error ({direction}): {exc}")
...
async def handle_client(client_reader: asyncio.StreamReader,
    ...
    print(f"[{PROXY_NAME}] session {session_id} from {peer}")
    ...
    try:
        ...
        print(f"[{PROXY_NAME}] connection error: {exc}")

    finally:
        client_writer.close()
        await client_writer.wait_closed()
        print(f"[{PROXY_NAME}] session closed: {session_id}")
